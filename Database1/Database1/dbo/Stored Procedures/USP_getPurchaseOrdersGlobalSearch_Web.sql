...
CREATE PROCEDURE [dbo].[USP_getPurchaseOrdersGlobalSearch_Web]         
...
--left join Companies CO on (Co.ParentCompanyID= POS.SupplierID or Co.companyid=POS.SupplierID)
WHERE PONumber LIKE 'PI%' AND (@SupplierId = 0 OR SupplierID IN (SELECT * FROM #Supplier))  
--OR Co.ParentCompanyID = @SupplierId or Co.companYid =@SupplierId)   
END  
