...
CREATE PROCEDURE [dbo].[USP_getPurchaseOrdersSearch_Web]
@SupplierId INT = 0,
...
SELECT DISTINCT
   ...
   ,(SELECT TOP 1 FieldValue FROM DefaultValues WHERE FieldName='CCMail') CCMail
   ,(SELECT TOP 1 FieldValue FROM DefaultValues WHERE FieldName='BCCMail') BCCMail
...
JOIN (SELECT Poid ,COUNT(poid) AS cnt ,MAX(DueDate) DueDate FROM PurchaseOrderLineItemsSync GROUP BY Poid) T
	ON T.poid = PO.poid
LEFT JOIN SupplierListWithShortName SSN
	ON PO.Supplier = SSN.Supplier
...
OUTER APPLY (SELECT STUFF((SELECT DISTINCT ',' + MU.Email
		FROM MasterUser MU
		JOIN Companies C1
		ON MU.GlobalId = C1.ParentCompanyID
		WHERE C1.CompanyID = PO.SupplierID
		AND ISNULL(MU.Email, '') <> ''
		FOR XML PATH ('')), 1, 1, '') SupplierEmails) SM
OUTER APPLY (SELECT
		M.POID
