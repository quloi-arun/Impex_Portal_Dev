...
CREATE PROCEDURE [dbo].[USP_getPurchaseOrdersSearch_Web_050423]  
@SupplierId INT = 0,  
...
SELECT DISTINCT  
 PO.POID  
   ...
   ,IIF(IIF(PO.LatestAction IS NULL, 'New', IIF(PO.PO_Status = 'FULLY RECEIVED, ACCEPTED', 'Completed', PO.LatestAction)) IN ('New','Updated','Viewed') 
		AND CreatedDate <= '2022-12-14','Accepted',
		IIF(PO.LatestAction IS NULL,'New',IIF(PO.PO_Status = 'FULLY RECEIVED, ACCEPTED', 'Completed', PO.LatestAction)))LatestAction	
   ,PO.Email  
