...
--USP_GetCargoDelayedEmail 3113307,219
CREATE PROCEDURE [dbo].[USP_GetCargoDelayedEmail_20230320] @POID INT, @SupplierInvoiceDocumentsID INT
AS  
BEGIN
...
<html>
   ...
   ,'PO- '+M.PONumber +'/'+left(DAtename(Month,M.PODate),3)+''+Right(Year(M.PODate),2)+' - '+M.Supplier+ ' Cargo Delay Notification .' [Subject]
FROM (
SELECT
		PO.PONumber
	   ,CO.CommentInfo
