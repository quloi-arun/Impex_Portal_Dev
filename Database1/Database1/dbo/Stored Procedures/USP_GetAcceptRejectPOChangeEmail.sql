...
CREATE PROCEDURE [dbo].[USP_GetAcceptRejectPOChangeEmail] @RequestIdentifier VARCHAR(20)
AS  
BEGIN
...
,(Select Top 1 SubSupplierName From SupplierName Where SupplierName = Trim(M.Supplier))
+ ' - ' + M.PONumber+'/'+Substring(DateName(MONTH,PODate),1,3)+Substring(DateName(YEAR,PODate),3,4)+' - Order change acceptation/ rejection' [Subject]
FROM (
SELECT
		PO.PONumber
	   ,CO.CommentInfo
