...
CREATE PROCEDURE [dbo].[USP_GetCommentEmailforPO] @POID INT
AS  
BEGIN
...
<html>
   ...
   ,(Select Top 1 SubSupplierName From SupplierName Where SupplierName = Trim(M.Supplier))+' - '+M.PONumber +'/'+SUBSTRING(DATENAME(MONTH,PODATE),1,3)+SUBSTRING(DATENAME(YEAR,PODATE),3,4)+ ' You have received new comment.' [Subject]
FROM (SELECT
		PO.PONumber
	   ,CO.CommentInfo
	   --,EM.TOMail
