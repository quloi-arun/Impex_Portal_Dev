...
CREATE PROCEDURE [dbo].[USP_GetAcceptRejectPriceChangeEmail] @RequestIdentifier VARCHAR(20)
AS  
BEGIN
...
+' - '+M.PONumber +'/'+SUBSTRING(DATENAME(MONTH,PODATE),1,3)+SUBSTRING(DATENAME(YEAR,PODATE),3,4)+ ' Price change request has been accepted/ rejected.' [Subject]
FROM (
SELECT
		PO.PONumber
	   ,CO.CommentInfo
