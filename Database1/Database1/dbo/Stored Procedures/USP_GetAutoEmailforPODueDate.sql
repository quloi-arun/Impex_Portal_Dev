CREATE PROCEDURE [dbo].[USP_GetAutoEmailforPODueDate]  
AS  
BEGIN
...
<html>
									...
									<table role="presentation" border="0" cellpadding="0" cellspacing="0" style=" border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; width: 100%;"width="100%" >
										<tr>
											<td style=" font-family: sans-serif; font-size: 14px; vertical-align: top;"valign="top" >
												<p style=" font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 6px;"> Dear ' + EM.Supplier + ', </p>
												<p style=" font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 6px;"> Purchase Order is due. Please find the purchase orders due as of today. </p>
												...
												<table style="border-collapse: collapse; width: 100%">
													<thead style="background-color: #eff4ff !important">
														...
														<th style="border: 1px solid #dddddd; text-align: left; padding: 8px; color: #3366ff;">PO NUMBER</th>
												...
												<p style=" font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 20px;"> <a href="',@Link,'impex/purchase-orders/', PO.POID, '">',@Link,'impex/purchase-orders/', PO.POID, '</a> </p>
   ...
   ,(Select Top 1 SubSupplierName From SupplierName Where SupplierName = Trim(EM.Supplier))+' - '+ PO.PONumber +'/'+SUBSTRING(DATENAME(MONTH,EM.PODATE),1,3)+SUBSTRING(DATENAME(YEAR,EM.PODATE),3,4)+ ' due purchase orders.' [Subject]
   ,PO.POID
FROM PurchaseOrdersSync PO
	LEFT JOIN (SELECT
		   ...
		   ,STUFF((SELECT
				',' + MU.Email
				FROM MasterUser MU
				WHERE MU.GlobalId IN 
				(SELECT DISTINCT C2.CompanyID FROM Companies C1
					LEFT JOIN Companies C2
						ON C1.ParentCompanyID = C2.ParentCompanyID
					WHERE C1.CompanyID = M.SupplierID)
				FOR XML PATH (''))
			, 1, 1, '') TOMail
		FROM (SELECT POID,Supplier,SupplierID,PODate
	...
	OUTER APPLY (SELECT
			STUFF((SELECT
				...
				FROM PurchaseOrdersDueHistoryEmail POS
				WHERE PO.POID = POS.POID
				FOR XML PATH (''))
			, 1, 1, '') PartNo) Part
CROSS APPLY (SELECT TOP 1 MailSentON FROM PurchaseOrdersDueHistoryEmail POH
WHERE PO.POID = POH.POID) POHM
WHERE POHM.MailSentON IS NULL
END