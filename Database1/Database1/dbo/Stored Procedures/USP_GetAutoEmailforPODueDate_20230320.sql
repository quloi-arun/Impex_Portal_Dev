...
Create PROCEDURE [dbo].[USP_GetAutoEmailforPODueDate_20230320]  
AS  
BEGIN
...
<html>
   ...
   ,PO.PONumber+'/'+left(DAtename(Month,PO.PODate),3)+''+Right(Year(PO.PODate),2)+' - '+PO.Supplier+ ' due purchase orders.' [Subject]
   ,PO.POID
FROM PurchaseOrdersSync PO
	...
	OUTER APPLY (SELECT
			STUFF((SELECT
				...
				FOR XML PATH (''))
			, 1, 1, '') PartNo) Part
OUTER APPLY (SELECT TOP 1 MailSentON FROM PurchaseOrdersDueHistoryEmail POH
