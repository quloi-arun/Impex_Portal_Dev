CREATE PROCEDURE [dbo].[USP_GetAcceptRejectPOChangeEmail_20230320(MaheshW)] @RequestIdentifier VARCHAR(20)
AS  
BEGIN
...
--			',' + ISNULL(MUI.Email,'')
--		FROM MasterUser MUI
--		WHERE MUI.GlobalId = ((SELECT MUI.GlobalId FROM PurchaseOrdersSync POS JOIN POChangeHistory PC 
--	ON PC.POID = POS.POID AND PC.RequestIdentifier = @RequestIdentifier))
--		FOR XML PATH (''))
--	, 1, 1, ''))
SELECT DISTINCT
									...
									<table role="presentation" border="0" cellpadding="0" cellspacing="0" style=" border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; width: 100%;"width="100%" >
										<tr>
											<td style=" font-family: sans-serif; font-size: 14px; vertical-align: top;"valign="top" >
												...
												<p style=" font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 6px;">',M.Supplier,', raised revision on order change has been accepted / rejected.</p>
												...
												<table style="border-collapse: collapse; width: 100%">
													<thead style="background-color: #eff4ff !important">
														...
														<th style="border: 1px solid #dddddd; text-align: left; padding: 8px; color: #3366ff;">Old Qty</th>
														<th style="border: 1px solid #dddddd; text-align: left; padding: 8px; color: #3366ff;">New Qty</th>','
														<th style="border: 1px solid #dddddd; text-align: left; padding: 8px; color: #3366ff;">Old Due Date</th>
														<th style="border: 1px solid #dddddd; text-align: left; padding: 8px; color: #3366ff;">New Due Date</th>
...
,M.PONumber+'/'+left(DAtename(Month,M.PODate),3)+''+Right(Year(M.PODate),2)+' - '+M.Supplier+' - Order change acceptation/ rejection' [Subject]
FROM (
SELECT
		PO.PONumber
	   ,CO.CommentInfo
	   ...
	   ,PO.PODate
	FROM PurchaseOrdersSync PO
	CROSS APPLY (SELECT DISTINCT PC.POID,PC.RequestIdentifier FROM POChangeHistory PC 
	...
	CROSS APPLY (SELECT DISTINCT 
			STUFF((SELECT
				...
				<td style="border: 1px solid #dddddd;text-align: left;padding: 8px;">', PCH.OldQty, '</td>
				<td style="border: 1px solid #dddddd;text-align: left;padding: 8px;">', PCH.NewQty, '</td>
				<td style="border: 1px solid #dddddd;text-align: left;padding: 8px;">', FORMAT(PCH.OldDueDate, 'dd-MM-yy'), '</td>
				<td style="border: 1px solid #dddddd;text-align: left;padding: 8px;">', FORMAT(PCH.NewDueDate, 'dd-MM-yy'), '</td>
