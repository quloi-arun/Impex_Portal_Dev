CREATE PROCEDURE [dbo].[Usp_GetImpexPortalNotification](@UserID INT)
AS
BEGIN
...
SELECT TOP 100 M.ID,IIF(ISNULL(M.InvoiceNo,'')='',M.ModuleName,'Invoice Screen')ModuleName,M.Header,M.Description,M.Action,M.isRead,RID.RedirectId,M.CreatedDate,M.Status,M.InvoiceNo,M.POID FROM (
...
END TODO,MU.UserType,IPAL.Status
,CASE WHEN Description LIKE 'Cargo ready for%' THEN REVERSE(SUBSTRING(REVERSE(Description),3,CHARINDEX('"',REVERSE(Description),3)-3))
WHEN Description LIKE 'Cargo is getting delay%' THEN REVERSE(SUBSTRING(REVERSE(Description),3,CHARINDEX('(',REVERSE(Description))-4))
WHEN Description LIKE 'Invoice (%' THEN SUBSTRING(Description,10,CHARINDEX(')',Description)-10) 
ELSE '' END InvoiceNo
,POS.POID
FROM ImpexPortalActionLog IPAL
LEFT JOIN PurchaseOrdersSync POS
ON POS.PONumber = IPAL.Header
LEFT JOIN MasterUser MU
...
ON M.ID = RID.ID
WHERE ((M.UserType='ImpexOperationAdmin' AND M.TODO IN ('NO','NSNT')) OR (M.UserType<>'ImpexOperationAdmin' AND M.TODO IN ('NS','NSNT')))
