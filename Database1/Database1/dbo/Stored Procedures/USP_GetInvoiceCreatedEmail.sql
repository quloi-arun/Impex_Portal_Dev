...
CREATE PROCEDURE [dbo].[USP_GetInvoiceCreatedEmail] @POID INT, @InvoiceNo VARCHAR(50)
AS  
BEGIN
...
<html>
									...
									<table role="presentation" border="0" cellpadding="0" cellspacing="0" style=" border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; width: 100%;"width="100%" >
										<tr>
											<td style=" font-family: sans-serif; font-size: 14px; vertical-align: top;"valign="top" >
												...
												<p style=" font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 6px;"> Thank you for placing your order with our company. We are glad to inform that invoice is now uploaded against this respective purchase order ' + M.PONumber + '. </p>
												...
												<table style="border-collapse: collapse; width: 100%">
													<thead style="background-color: #eff4ff !important">
														...
														<th style="border: 1px solid #dddddd; text-align: left; padding: 8px; color: #3366ff;">Invoice Created By</th>
														<th style="border: 1px solid #dddddd; text-align: left; padding: 8px; color: #3366ff;">Invoice Date</th>
													</thead>
													<tbody>
														<tr>
   ...
   ,(Select Top 1 SubSupplierName From SupplierName Where SupplierName = Trim(M.Supplier))+' - '+M.PONumber +'/'+SUBSTRING(DATENAME(MONTH,PODATE),1,3)+SUBSTRING(DATENAME(YEAR,PODATE),3,4)+ ' Invoice has been generated.' [Subject]
FROM (
SELECT
		PO.PONumber
	   ,CO.CommentInfo
	...
	CROSS APPLY (SELECT DISTINCT 
			STUFF((SELECT
				...
				<td style="border: 1px solid #dddddd;text-align: left;padding: 8px;">', FORMAT(SID.CreatedDate, 'dd-MM-yy'), '</td>
			  </tr>')
				...
				WHERE PO.POID = SID.POID AND SID.InvoiceNo = @InvoiceNo
				FOR XML PATH (''))
			, 1, 1, '') CommentInfo) CO
			WHERE PO.POID = @POID
