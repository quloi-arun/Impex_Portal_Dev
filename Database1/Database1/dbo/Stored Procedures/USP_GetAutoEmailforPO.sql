...
CREATE PROCEDURE [dbo].[USP_GetAutoEmailforPO]  
AS  
BEGIN
...
<html>
   ...
   +' - '+'PO NO-' + M.PONumber+'/'+SUBSTRING(DATENAME(MONTH,PODATE),1,3)+SUBSTRING(DATENAME(YEAR,PODATE),3,4) + ' Is waiting for your response' [Subject]
   ,M.ActionHistoryId
FROM (SELECT
		PO.PONumber
	   ,PO.DueDate
	...
	WHERE PO.LatestAction IN ('New', 'Updated', 'Viewed')
	AND ISNULL(PO.isDeleted,0) = 0) M
WHERE M.Trg = 1
END